...
- [**Report** (HTML)](docs/report.html) — The same writeup, published alongside the demo

## 🔗 Live Demo

The analysis is also published as a self-contained static page, built from this
repository and served by GitHub Pages:

**https://tewei02.github.io/From-Signal-to-System-Communications-Sensing-and-Robotic-Navigation/**

Three models from the repository run in the browser, next to the Python code that
defines them:

| Panel | What it shows | Python source |
|-------|---------------|---------------|
| Multi-robot exploration | Coverage per step for a full-map upload and for differential updates under a per-step cell budget | `tools/coverage_model.py` |
| Latency vs. navigation error | Dead-band displacement `v·τ` against the 0.1 m tolerance, with a jitter band | `analysis/plot_latency_vs_nav_error.py` |
| LiDAR link budget | Required uplink for raw, downsampled and occupancy-difference streams | `analysis/plot_bandwidth_vs_update_rate.py` |

The page re-derives every series from the same fixed seed and compares the result
with `docs/data/reference.json`, which is generated by Python. The verification
strip at the top of the page turns green only when all 14 comparisons agree
bit-for-bit; the panel below it lists each comparison and its sample count.

Every figure on the page is model output under seed `20260922`, or a data-sheet
order-of-magnitude assumption that is labelled as such. **There is no hardware and
no measurement in this repository** — the indoor case study is hypothetical and is
presented as a design argument rather than a validation.
...
# Install Python dependencies used by the simulators and the analysis scripts
...
### Reproduce the Published Page

Two files under `docs/` are generated rather than hand-written, and both can be
rebuilt from the repository:

```bash
# docs/data/reference.json — the dataset the browser demo verifies itself against
python tools/build_web_data.py

# docs/report.html — the HTML rendering of report/report.tex
python tools/build_report.py

# serve the published build locally
python -m http.server -d docs 8000
```

`tools/coverage_model.py` is the portable model shared by Python and the browser;
`tools/verify_engine.js` re-runs `docs/engine.js` against `docs/data/reference.json`
outside a browser (JavaScriptCore, no dependencies).
...
This project is a **design study**: a reference architecture for exploring the
intersection of communications and autonomous navigation. The indoor case study is
hypothetical, the reported figures are analytical results or data-sheet
order-of-magnitude assumptions, and the code is **not intended for production use**.
