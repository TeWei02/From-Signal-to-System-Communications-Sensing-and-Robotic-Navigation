...
This is a design study: the case study is hypothetical, the reported figures are analytical
results or data-sheet order-of-magnitude assumptions, and no experimental measurement is claimed.
\end{abstract}
...
Within the design argument followed here, adaptive sampling strategies---reducing sensor
update rates when the environment is static, or increasing resolution near obstacles---are
expected to maintain navigation accuracy while reducing the data volume that must be
processed or transmitted downstream; quantifying that expectation is left to future work.
...
budget.
The design consequence is that algorithm complexity and hardware capability must be
...
A single full-resolution LiDAR scan of 100\,000 points at three 4-byte coordinates each
occupies about 1.2\,MB, i.e.\ roughly 100\,Mbps if streamed at 10\,Hz uncompressed---far
beyond a congested Wi-Fi cell shared with several other users. The analysis scripts in
this repository therefore start from a 50\,Mbps budget for a raw but compressed stream,
5\,Mbps for a downsampled stream, and 0.5\,Mbps for an occupancy-difference stream.
...
The first-order model used here gives a dead-band displacement that grows linearly with
mean latency; real systems are expected to degrade more steeply once \(\tau\) exceeds a
threshold that depends on the controller and on how quickly the environment changes.
...
Sending semantically compressed updates---for example, changed cells in an occupancy
grid rather than a full map snapshot---is the natural way to obtain bandwidth savings
without discarding the information the planner actually consumes.
...
In the scenario assumed here the frontier of newly explored space is a thin boundary, so
we take fewer than 5\% of the cells to change between consecutive scans; the portable model
in \texttt{tools/coverage\_model.py} measures this quantity directly.
...
The grid-world model released with this report reproduces that ordering on synthetic
occupancy grids: differential updates reach the coverage of a full upload once the
per-step cell budget exceeds the rate at which the robots learn new cells, and fall well
behind when the budget sits below it.
